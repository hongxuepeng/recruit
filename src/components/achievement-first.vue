    ...
    methods: {
      ...
      async onLoad() {
        try {
          ...
          let res = ''
          if (this.type === 1) {
            res = await getMyLowLoanList(params)
          } else {
            res = await getMyLowLowLoanList(params)
          }
          if (+res.code === 1) {
            let myList = this.type === 1 ? res.info.myLowLoanList : res.info.myLowLowLoanList
